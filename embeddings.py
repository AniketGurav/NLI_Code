...
def create_embeddings(df_data, glove_dict, batch_i, batch_size, test_tbir=False, max_pre=45, max_hypo=45):
    ...
    if test_tbir is False:
        ...
        for i in range(batch_i, limit): #zip(list_premises, list_hypothesis, list_label, list_annotator):
            ...
            premise_encoded=create_embedding_sentence(list_premises[i], glove_dict, maxlen=max_pre)
            hypothesis_encoded=create_embedding_sentence(list_hypothesis[i], glove_dict, maxlen=max_hypo) # do the cut here whether it has same labels or not
            if premise_encoded is not False and hypothesis_encoded is not False:
